...
void UpdaterWindow::OnCancel()
{
	if (JUpdater_Updatering()) {
		if (MessageBox(_hWnd, L"正在下載更新，確定要取消升級嗎？", L"提示", MB_ICONINFORMATION | MB_YESNO) == IDYES)
		{
...
void UpdaterWindow::OnUpdateDownloadCallback(LPCWSTR precent, int status)
{
	if (status == UPDATE_STATUS_COULD_NOT_CONNECT) {
		progress_text.set_text(L"更新下載失敗！");
		MessageBox(_hWnd, L"無法連線至更新伺服器", L"JiYuTrainer - 更新错误", MB_ICONEXCLAMATION);
		SendMessage(_hWnd, WM_COMMAND, IDC_UPDATE_CLOSE, NULL);
	}
	else if (status == UPDATE_STATUS_COULD_NOT_CREATE_FILE) {
		progress_text.set_text(L"更新安裝失敗！");
		MessageBox(_hWnd, L"無法寫入更新檔案，請嘗試以系統管理員身分執行本程式", L"JiYuTrainer - 更新错误", MB_ICONEXCLAMATION);
		SendMessage(_hWnd, WM_COMMAND, IDC_UPDATE_CLOSE, NULL);
	}
	else if (status == UPDATE_STATUS_DWONLAODING) {
		if (!setDt1) {
			progress_text.set_text(L"正在下載更新：");
			setDt1 = true;
		}
		updateProgressPrect = precent;
	}
	else if (status == UPDATE_STATUS_FINISHED) {
		progress_text.set_text(L"更新下載完成！");
		SendMessage(_hWnd, WM_COMMAND, IDC_UPDATE_CLOSE, NULL);
