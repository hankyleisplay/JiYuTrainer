...
INT_PTR CALLBACK BugReportDlgFunc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		SetDlgItemText(hDlg, IDC_BUGREPORT_CONTENT, L"無法取得程式錯誤記錄");
	...
	case WM_COMMAND: {
		...
		case IDOK: {
			if (JUpdater_CheckInternet()) {
				Sleep(2000);
				MessageBox(hDlg, L"您的錯誤回報已成功送出，感謝您的支持！", L"JiYuTrainer", MB_ICONINFORMATION);
				EndDialog(hDlg, LOWORD(wParam));
			}
			else {
				MessageBox(hDlg, L"無法送出錯誤回報，可能是網路未連線？", L"JiYuTrainer", MB_ICONEXCLAMATION);
			}
