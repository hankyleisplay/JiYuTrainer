...
ScanIPWindow::ScanIPWindow(HWND parentHWnd) : CommonWindow(parentHWnd, 330, 360, L"JiYuTrainerScanIPWindow", L"掃描區域網路 IP", IDR_HTML_SCANIP)
{
...
void ScanIPWindow::ScanFinish() {
	...
	btn_refresh_ip_list.set_text(L"掃描完成，重新掃描");
	btn_refresh_ip_list.remove_attribute("disabled");

	if (scanCount == 0)
		AddStringToIpList(L"未搜尋到區域網路內的主機");
}
...
void ScanIPWindow::RefreshIpList() {
	...
	if (!std::regex_match(start.begin(), start.end(), express)) {
		MessageBox(_hWnd, L"起始位址不正確，僅支援 IPv4 位址格式 (xx.xx.xx.xx)", L"提示", MB_ICONEXCLAMATION);
		return;
	}
	if (!std::regex_match(end.begin(), end.end(), express)) {
		MessageBox(_hWnd, L"結束位址不正確，僅支援 IPv4 位址格式 (xx.xx.xx.xx)", L"提示", MB_ICONEXCLAMATION);
		return;
	}
	...
	if (ipHost1 != ipHost2) {
		MessageBox(_hWnd, L"起始位址與結束位址必須在同一個子網路區段", L"提示", MB_ICONEXCLAMATION);
		return;
	}
	...
	scanCount = 0;
	ip_list.clear();
	btn_refresh_ip_list.set_text(L"正在掃描，請稍候...");
