...
BOOL MUnLoadDriverServiceWithMessage(const wchar_t* szSvrName)
{
	...
	if (hServiceMgr == NULL)
	{
		...
		FAST_STR_BINDER(str, L"卸載驅動程式錯誤，開啟驅動程式管理失敗：%s\n請嘗試以系統管理員身分執行軟體。", 128, SysHlp::ConvertErrorCodeToString(lastErr));
		MessageBox(NULL, str, L"JiYuTrainer - 錯誤", MB_ICONERROR);
		bRet = FALSE;
		goto BeforeLeave;
	}
	//打开驱动所对应的服务
	...
	if (hServiceDDK == NULL)
	{
		...
		if (lastErr == ERROR_SERVICE_DOES_NOT_EXIST) 
			MessageBox(NULL, L"驅動程式已卸載並刪除，請勿重複操作", L"JiYuTrainer - 提示", MB_ICONEXCLAMATION);
		else if ( lastErr == ERROR_SERVICE_MARKED_FOR_DELETE) 
			MessageBox(NULL, L"未在此電腦上找到該驅動程式，可能已被卸載", L"JiYuTrainer - 提示", MB_ICONEXCLAMATION);
		else {
			FAST_STR_BINDER(str, L"卸載驅動程式錯誤，開啟驅動程式失敗：%s", 128, SysHlp::ConvertErrorCodeToString(lastErr));
			MessageBox(NULL, str, L"JiYuTrainer - 錯誤", MB_ICONERROR);
		}
		bRet = FALSE;
		goto BeforeLeave;
	...
	if (!ControlService(hServiceDDK, SERVICE_CONTROL_STOP, &SvrSta)) {
		...
		if (lastErr == ERROR_SERVICE_MARKED_FOR_DELETE) {
			...
			FAST_STR_BINDER(str, L"卸載驅動程式錯誤，停止驅動程式失敗：%s", 128, SysHlp::ConvertErrorCodeToString(lastErr));
			MessageBox(NULL, str, L"JiYuTrainer - 錯誤", MB_ICONERROR);
		}
	}
	//动态卸载驱动程序。 
	if (!DeleteService(hServiceDDK)) {
		...
		if (lastErr == ERROR_SERVICE_MARKED_FOR_DELETE) 
			...
			FAST_STR_BINDER(str, L"卸載驅動程式錯誤，刪除驅動程式失敗：%s", 128, SysHlp::ConvertErrorCodeToString(lastErr));
			MessageBox(NULL, str, L"JiYuTrainer - 錯誤", MB_ICONERROR);
			bRet = FALSE;
		}
	}
...
BOOL XTestDriverCanUse() {
	...
	if (SysHlp::Is64BitOS()) {
		currentLogger->LogWarn(L"驅動程式不支援 64 位元系統");
		return FALSE;
	}
	if (!SysHlp::IsRunasAdmin() && !isXp) {
		currentLogger->LogWarn(L"若要載入驅動程式，請以系統管理員身分執行本程式");
		return FALSE;
	}
...
BOOL XLoadDriver() {
	...
	if (MLoadKernelDriver(L"JiYuTrainerDriver", currentApp->GetPartFullPath(PART_DRIVER), NULL))
	{
		if (XOpenDriver()) {
			...
			currentLogger->LogInfo(L"驅動程式載入成功");
			return TRUE;
		}
		else currentLogger->LogWarn2(L"驅動程式載入成功，但打开驱动失败");
	}
