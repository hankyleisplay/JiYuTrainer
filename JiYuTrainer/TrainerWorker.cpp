...
void TrainerWorkerInternal::Init()
{
	...
	if (LocateStudentMainLocation()) currentLogger->Log(L"已定位極域電子教室路徑： %s", _StudentMainPath.c_str());
	else currentLogger->LogWarn(L"無法定位極域電子教室路徑");
...
bool TrainerWorkerInternal::Kill(bool autoWork)
{
	if (_StudentMainPid <= 4) {
		currentLogger->LogError(L"未找到極域主處理程序");
		return false;
	}
	if (_StudentMainControlled){
		bool vkill = autoWork;
		if(!vkill){
			...
			mbp.lpszText = L"您是否希望使用注入模組進行強制解除？";
	...
	if (!NT_SUCCESS(status)) {
		if (status == STATUS_INVALID_CID || status == STATUS_INVALID_HANDLE) {
			...
			currentLogger->LogError(L"開啟處理程序錯誤：0x%08X，請手動結束", status);
			return false;
		}
	}
	...
	if (NT_SUCCESS(status)) {
		...
		if (status == STATUS_ACCESS_DENIED) goto FORCEKILL;
			...
			currentLogger->LogError(L"結束處理程序錯誤：0x%08X，請手動結束", status);
			if (!autoWork)
				MessageBox(hWndMain, L"無法結束極域電子教室，您需要使用其他工具手動結束", L"JiYuTrainer - 錯誤", MB_ICONERROR);;
			CloseHandle(hProcess);
			return false;
		}
		else if (status == STATUS_INVALID_CID || status == STATUS_INVALID_HANDLE) {
...
FORCEKILL:
	if (XDriverLoaded() && MessageBox(hWndMain, L"常規方式無法結束極域，是否調用驅動程式強制結束？\n（驅動程式可能不穩定，請慎用。您亦可使用 PCHunter 等工具進行強制結束）", L"JiYuTrainer - 提示", MB_ICONEXCLAMATION | MB_YESNO | MB_TOPMOST) == IDYES)
	{
		...
		else if(!autoWork) MessageBox(hWndMain, L"驅動程式亦無法結束，請使用 PCHunter 結束它！", L"错误", MB_ICONEXCLAMATION);
	}
	CloseHandle(hProcess);
	return false;
}
...
bool TrainerWorkerInternal::Rerun(bool autoWork)
{
	if (!_StudentMainFileLocated) {
		currentLogger->LogWarn(L"未找到極域電子教室");
		if (!autoWork && _Callback)
			_Callback->OnSimpleMessageCallback(L"<h5>無法在此電腦上找到極域電子教室，您需要手動啟動</h5>");
		return false;
	}
...
void TrainerWorkerInternal::RunResetPid()
{
	...
	DWORD newPid = 0;
	if (LocateStudentMain(&newPid)) { //找到極域
		...
		if (_StudentMainPid != newPid)
		{
			...
			if (InstallVirus()) {
				...
				currentLogger->Log(L"向 StudentMain.exe [%d] 注入 DLL 成功", newPid);
			}
			else  currentLogger->LogError(L"向 StudentMain.exe [%d] 注入 DLL 失敗", newPid);

			currentLogger->Log(L"已鎖定 StudentMain.exe [%d]", newPid);
		...
		if (_StudentMainPid != 0)
		{
			...
			currentLogger->Log(L"極域主處理程序 StudentMain.exe 已結束", newPid);
	...
	if (LocateMasterHelper(&newPid)) {
		if (_MasterHelperPid != newPid)
		{
			...
			else  currentLogger->LogError(L"向 MasterHelper.exe [%d] 注入 DLL 失敗", newPid);
		}
	}
	else {
		_MasterHelperPid = 0;
...
bool TrainerWorkerInternal::KillProcess(DWORD pid, bool force)
{
	...
	if (!NT_SUCCESS(status)) {
		if (status == STATUS_INVALID_CID || status == STATUS_INVALID_HANDLE) {
			currentLogger->LogError2(L"找不到處理程序 [%d] ", pid);
			return true;
		}
		else {
			currentLogger->LogError2(L"開啟處理程序 [%d] 錯誤：0x%08X，請手動結束", pid);
			return false;
		}
	}
	status = MTerminateProcessNt(0, hProcess);
	if (NT_SUCCESS(status)) {
		currentLogger->Log(L"處理程序 [%d] 結束成功", pid);
		CloseHandle(hProcess);
		return TRUE;
	}
	else {
		if (status == STATUS_ACCESS_DENIED) {
			...
			else currentLogger->LogError2(L"結束處理程序 [%d] 錯誤：存取被拒。可嘗試使用驅動程式結束", pid);
			CloseHandle(hProcess);
		}
		else if (status != STATUS_INVALID_CID && status != STATUS_INVALID_HANDLE) {
			currentLogger->LogError2(L"結束處理程序 [%d] 錯誤：0x%08X，請手動結束", pid);
			CloseHandle(hProcess);
			return false;
		}
		else if (status == STATUS_INVALID_CID || status == STATUS_INVALID_HANDLE) {
...
FORCEKILL:
	...
	{
		if (KForceKill(_StudentMainPid, &status)) {
			currentLogger->Log(L"處理程序 [%d] 強制結束成功", pid);
			CloseHandle(hProcess);
			return true;
		}
		else {
			currentLogger->LogError2(L"驅動程式強制結束處理程序 [%d] 錯誤：0x%08X", pid);
		}
	}
	else currentLogger->Log(L"驅動程式未載入，無法強制結束處理程序");
	CloseHandle(hProcess);
	return false;
}
...
bool TrainerWorkerInternal::AppointStudentMainLocation(LPCWSTR fullPath) {
	...
	if (Path::Exists(fullPath)) 
	{
		...
		currentLogger->Log(L"成功手動定位極域電子教室路徑： %s", fullPath);
...
bool TrainerWorkerInternal::LocateStudentMainLocation()
{
	...
	if (MRegReadKeyString64And32(HKEY_LOCAL_MACHINE,
		...
		else currentLogger->Log(L"讀取登錄檔 [DisplayIcon] 取得無效的極域電子教室路徑 : %s", Data);
	}
	else currentLogger->LogWarn2(L"MRegReadKeyString64And32 Failed : %s (%d)", PRINT_LAST_ERROR_STR);
	...
	LPCWSTR mabeInHere[6] = {
		L"c:\\Program Files\\Mythware\\極域課堂管理系統軟體V6.0 2016 豪華版\\StudentMain.exe",
				...
				if (!_StudentMainFileLocated) {
					...
					if (NT_SUCCESS(MOpenProcessNt((DWORD)p->ProcessId, &hProcess))) {
						...
						if (MGetProcessFullPathEx(hProcess, buffer)) {
							...
							_StudentMainFileLocated = true;
							currentLogger->Log(L"透過處理程序 StudentMain.exe [%d] 定位到路徑： %s", (DWORD)p->ProcessId, _StudentMainPath);
							CloseHandle(hProcess);
							return true;
						}
						CloseHandle(hProcess);
...
bool TrainerWorkerInternal::InjectDll(DWORD pid, LPCWSTR dllPath)
{
	...
	if (!NT_SUCCESS(ntStatus)) {
		currentLogger->LogError2(L"注入模組失敗！開啟處理程序失敗：0x%08X", ntStatus);
		return FALSE;
	}
	...
	{
		currentLogger->LogError2(L"注入執行緒失敗！錯誤：CreateRemoteThread %d", GetLastError());
		return FALSE;
	}
...
bool TrainerWorkerInternal::UnInjectDll(DWORD pid, LPCWSTR moduleName)
{
	...
	if (!NT_SUCCESS(ntStatus)) {
		currentLogger->LogError2(L"卸載模組失敗！開啟處理程序失敗：0x%08X", ntStatus);
		return FALSE;
	}
	...
	if (!hThread) {
		currentLogger->LogError2(L"卸載模組失敗！建立遠端執行緒失敗：%d", GetLastError());
		return FALSE;
	}
...
void TrainerWorkerInternal::FakeFull(bool fk) {
	if (_CurrentBroadcastWnd)
	{
		if (fk) {
			...
			currentLogger->Log(L"調整廣播視窗偽裝全螢幕狀態");
		}
		else {
			_FakeBroadcastFull = false;
			FixWindow(_CurrentBroadcastWnd, (LPWSTR)L"螢幕廣播");
			...
			currentLogger->Log(L"取消廣播視窗偽裝全螢幕狀態");
		}
	}
...
bool TrainerWorkerInternal::CheckWindowTextIsGb(const wchar_t* text) {
	return StringHlp::StrContainsW(text, L"广播", nullptr) || StringHlp::StrContainsW(text, L"廣播", nullptr)
		|| StringHlp::StrContainsW(text, L"演示", nullptr)
		|| StringHlp::StrContainsW(text, L"共享", nullptr)
		|| StringHlp::StrEqualW(text, L"屏幕演播室窗口") || StringHlp::StrEqualW(text, L"螢幕演播室視窗");
}
