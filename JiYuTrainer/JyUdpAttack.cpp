...
DWORD __stdcall JyUdpAttack::SendThread(void* data) {
    ...
    TaskFeedbackMessage(task, new std::wstring(L"開始發送"));
    ...
    if (sockfd > 0) {
        ...
        if ((sendBytes = sendto(sockfd,
            ...
            TaskFeedbackMessage(task, FormatStringPtr(L"send 失敗：%d", errno));
        else 
            TaskFeedbackMessage(task, FormatStringPtr(L"發送成功，共 %d 位元組", sendBytes));
           
        closesocket(sockfd);
    }
    else 
        TaskFeedbackMessage(task, FormatStringPtr(L"socket 失敗：%d", errno));
