...
BOOL GenerateCrashInfo(PEXCEPTION_POINTERS pExInfo, LPCWSTR info_file_name, LPCWSTR file_name, SYSTEMTIME tm, LPCWSTR dir) {
	...
	currentLogger->LogError(L"應用程式當機：Address : 0x%08x  Code : 0x%08X  (0x%08X)",
	...
	currentLogger->LogError(L"已建立記憶體傾印檔案：%s", file_name);
	...
	if (fp) {
		...
		fwprintf_s(fp, L"\r\n應用程式錯誤：Address : 0x%08x  Code : 0x%08X  (0x%08X)",
			(ULONG_PTR)pExInfo->ExceptionRecord->ExceptionAddress, pExInfo->ExceptionRecord->ExceptionCode,
			pExInfo->ExceptionRecord->ExceptionFlags);
		fwprintf_s(fp, L"\r\n=== JiYuTrainer =====================================");
		fwprintf_s(fp, L"\r\n已產生描述此問題的錯誤報告（不包含個人隱私資料）：");
		fwprintf_s(fp, L"\r\n=== 檔案內容 =====================================");
		fwprintf_s(fp, L"\r\n[錯誤傾印檔案] %s", file_name);
		fwprintf_s(fp, L"\r\n[程式執行記錄] %s\\JiYuTrainer.log", dir);
		fwprintf_s(fp, L"\r\n=== %hs =================================", CURRENT_VERSION);
		fclose(fp);
		return TRUE;
	}
...
LONG GenerateMiniDump(PEXCEPTION_POINTERS pExInfo)
{
	...
	SYSTEMTIME tm;
	GetLocalTime(&tm);//取得時間
	TCHAR file_name[128];
	swprintf_s(file_name, L"%s\\JiYuTrainerCrashDump%d%02d%02d-%02d%02d%02d.dmp", dmp_path,
		tm.wYear, tm.wMonth, tm.wDay, tm.wHour, tm.wMinute, tm.wSecond);//設定 dmp 檔案名稱
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		TCHAR info[300];
		swprintf_s(info, L"應用程式發生錯誤，%s。\n%s", (hasCrashInfo ? L"需要關閉，已產生關於此錯誤的報告" : L"且無法建立錯誤傾印檔案"), expInfo);
		MessageBoxTimeoutW(NULL, info, L"JiYuTrainer 應用程式錯誤", MB_ICONERROR | MB_SYSTEMMODAL, 0, 3600);
	...
	{
		TCHAR info[300];
		swprintf_s(info, L"应用程序出现了一个错误，且無法建立錯誤傾印檔案。\n%s\nFail to create dump file: %s \nLast Error : %d\n现在应用程序即将关闭。", expInfo, file_name, GetLastError());
		MessageBox(NULL, info, L"JiYuTrainer 應用程式錯誤", MB_ICONERROR | MB_SYSTEMMODAL);
	}
...
EXPORT_CFUNC(VOID) ShowAppStartupFail(int rs) {
	if (rs == APP_FAIL_SYSTEM_NOT_SUPPORT)
		MessageBox(NULL, L"執行本程式最低系統需求為 Windows XP，請使用更新版本的作業系統", L"JiYuTrainer - 錯誤", MB_ICONERROR);
	else if (rs == APP_FAIL_ALEDAY_RUN)
		MessageBox(0, L"已有另一個程式執行個體正在運作，同一時間僅能執行一個執行個體，請先關閉前一個", L"JiYuTrainer - 錯誤", MB_ICONERROR);
	else if (rs == APP_FAIL_PIRACY_VERSION)
		MessageBox(0, L"您可能是未授權修改軟體的受害者，目前執行的是被竄改過的 JiYuTrainer，建議您至官方 GitHub 專案下載安全正版。", L"JiYuTrainer - 錯誤", MB_ICONERROR);
	else if (rs == APP_FAIL_INSTALL) {
		FAST_STR_BINDER(err, L"安裝失敗，詳細錯誤請參閱記錄檔：\n%s", 300, currentApp->GetPartFullPath(PART_LOG));
		MessageBox(0, err, L"JiYuTrainer - 錯誤", MB_ICONERROR);
	}
}
