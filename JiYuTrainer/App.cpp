...
#define CMD_HELP L"\
工具指令：\n\
-install-full 開始完整安裝指令\n\
-config       開啟 JiYuTrainer 進階設定\n\
-hidden       靜音背景執行模式\n\
-killst       強制結束極域電子教室\n\
\n\
除錯指令：\n\
...
int JTAppInternal::CheckAndInstall()
{
	...
	if (!appIsInstaller && !appForceIntallInCurrentDir && SysHlp::CheckIsPortabilityDevice(fullDir.c_str()))
	{
		...
		if (!Path::Exists(szTempPath) && !CreateDirectory(szTempPath, NULL)) {
			appLogger->LogError2(L"建立暫存目錄失敗：%s (%d)", PRINT_LAST_ERROR_STR);
			return -1;
		}
		...
		if (!CopyFile(fullPath.c_str(), szTempMainPath, FALSE)) {
			appLogger->LogError2(L"建立主程式失敗：%s (%d)", PRINT_LAST_ERROR_STR);
			return -1;
		}
		...
		if (!SysHlp::RunApplicationPriviledge(L"cmd", runBatContent.c_str())) {
			appLogger->LogError2(L"啟動主程式失敗：%s (%d)", PRINT_LAST_ERROR_STR);
			return -1;
		}
		return 0;
	}
	...
	if (!LoadLibrary(fullSciterPath.c_str())) {
		appLogger->LogError2(L"模組 Sciter.dll 載入失敗：%s (%d)", PRINT_LAST_ERROR_STR);
		return -1;
	}
	...
	if (appIsInstaller) {
		...
		if (Path::Exists(mainExePath) && !DeleteFile(mainExePath.c_str())) {
			appLogger->LogError2(L"無法更新原始主 exe ：%s (%d)", PRINT_LAST_ERROR_STR);
			return -1;
		}
		//更新来源exe
		if (Path::Exists(fullSourceInstallerPath) && DeleteFile(fullSourceInstallerPath.c_str()) && !CopyFile(fullPath.c_str(), fullSourceInstallerPath.c_str(), TRUE)) 
			appLogger->LogError2(L"無法更新來源 exe ：%s (%d) %s", PRINT_LAST_ERROR_STR, fullSourceInstallerPath.c_str());
		if (CopyFile(fullPath.c_str(), mainExePath.c_str(), TRUE)) {
			...
			appLogger->LogError2(L"建立主 exe 失敗：%s (%d) %s", PRINT_LAST_ERROR_STR, mainExePath.c_str());
			return -1;
		}
	}
	else {
		...
		appLogger->LogError2(L"讀取檔案失敗，資源解壓錯誤：%s (%d)", PRINT_LAST_ERROR_STR);
	}
...
int JTAppInternal::RunInternal()
{
	...
	if (appKillStMode) {
		...
		if (t.KillStAuto()) printf_s("已成功結束極域電子教室\n");
		else printf_s("無法結束極域電子教室，詳細資訊請參閱記錄檔\n");
		return 0;
	}
...
INT_PTR CALLBACK JTAppInternal::AVTipWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		SetDlgItemText(hDlg, IDC_MESSAGE, FormatString(L"偵測到您的電腦安裝了 %s 防毒軟體，由於 JiYuTrainer 會對極域進行防護操作，可能會被防毒軟體誤判為病毒。\n因此建議您暫時關閉防毒軟體或將本軟體加入白名單。", ((JTAppInternal*)lParam)->existsAntiVirus.c_str()).c_str());
		lResult = TRUE;
		break;
	}
	case WM_COMMAND: 
...
INT_PTR CALLBACK JTAppInternal::ArgeementWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		SendDlgItemMessage(hDlg, IDC_STATIC_RED, WM_SETFONT, (WPARAM)hFontRed, TRUE);//發送設定字型訊息
