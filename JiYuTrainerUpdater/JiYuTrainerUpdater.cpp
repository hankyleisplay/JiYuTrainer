// JiYuTrainerUpdater.cpp : 定義應用程式的進入點。
//
...
int CheckServerForUpdateInfo(bool byUser) {
	...
	if (res != CURLE_OK) {
		appLogger->LogError(L"檢查更新錯誤：curl_easy_perform() failed:  %S", curl_easy_strerror(res));
		...
		if (getResponseStr == "newupdate")
			...
			appLogger->LogError(L"檢查更新錯誤：update service return bad result :  %S", getResponseStr.c_str());
...
BOOL CheckServerForUpdateNew(LPCSTR buffer, size_t maxCount) {
	...
	auto res = curl_get_req(getUrlStr, getResponseStr);
	if (res != CURLE_OK) {
		appLogger->LogError(L"取得更新內容錯誤：curl_easy_perform() failed:  %S", curl_easy_strerror(res));
		return FALSE;
	}
	else {
...
UPEXPORT_CFUNC(BOOL) JUpdater_DownLoadUpdateFile(UpdateDownloadCallback callBack, LPARAM lpararm)
{
	...
	auto res = curl_get_req(getUrlStr, getResponseStr);
	if (res != CURLE_OK) {
		appLogger->LogError(L"取得更新錯誤 : curl_easy_perform() failed:  %S", curl_easy_strerror(res));
	...
	else if (getResponseStr != "")
	{
		...
		appLogger->LogError(L"取得更新錯誤，空返回值");
		callBack(0, callBackLparam, UPDATE_STATUS_COULD_NOT_CONNECT);
	}
...
DWORD WINAPI UpdateDownloadThread(LPVOID lpThreadParameter)
{
	...
	if (curl)
	{
		...
		if (!updateFile) {
			appLogger->LogError(L"建立更新檔案錯誤 : fopen:  %d", err);
		...
		if (res != CURLE_OK)
		{
			appLogger->LogError(L"下載更新檔案錯誤 %s 失败 :  %S", url, curl_easy_strerror(res));
		...
		appLogger->LogError(L"下載更新檔案錯誤 : curl failed ");
