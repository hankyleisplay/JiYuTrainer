...
void VParamInit() {
	...
	AppendMenu(hMenuGb, MF_STRING , IDM_HELP_GB, L"JiYuTrainer 新增的選單");
	AppendMenu(hMenuGb, MF_STRING, IDM_HELP_GB2, L"JiYuTrainer 說明");
	AppendMenu(hMenuGb, MF_SEPARATOR, 0, NULL);
	AppendMenu(hMenuGb, MF_STRING, IDM_FULL, L"廣播視窗全螢幕");
	AppendMenu(hMenuGb, MF_STRING, IDM_TOPMOST, L"廣播視窗置頂");
	AppendMenu(hMenuGb, MF_SEPARATOR, 0, NULL);
	AppendMenu(hMenuGb, MF_STRING, IDM_CLOSE_GB, L"關閉廣播視窗");
...
void VFixGuangBoWindow(HWND hWnd)
{
	...
	AppendMenu(hMenu, MF_STRING, IDM_FULL, L"廣播視窗全螢幕");
	AppendMenu(hMenu, MF_STRING, IDM_TOPMOST, L"廣播視窗置頂");
...
INT_PTR CALLBACK VShowOpConfirmWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER: {
		if (wParam == TIMER_COUNTDOWN_TICK) {
			...
			if (w > 0) {
				WCHAR str[18]; swprintf_s(str, L"拒絕 (%d)", w); 
...
INT_PTR CALLBACK VShowOutOfControlWndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER: {
		if (wParam == TIMER_COUNTDOWN_TICK) {
			...
			if (w > 0) {
				WCHAR str[18]; swprintf_s(str, L"重新啟動主程式 (%d)", w);
...
BOOL WINAPI hkExitWindowsEx(UINT uFlags, DWORD dwReason) {
	const wchar_t* op = L"";
	if((uFlags & EWX_POWEROFF) == EWX_POWEROFF) op = L"關閉您的電腦";
	else if ((uFlags & EWX_REBOOT) == EWX_REBOOT) op = L"重新啟動您的電腦";
	else if ((uFlags & EWX_LOGOFF) == EWX_LOGOFF) op = L"登出您的電腦";
	wchar_t str[37];
	swprintf_s(str, L"極域電子教室試圖%s，是否允許極域繼續操作？", op);
	if (MessageBox(NULL, str, L"JiYu Killer 防護警告", MB_ICONEXCLAMATION | MB_YESNO) == IDYES) 
...
BOOL WINAPI hkCreateProcessW(LPCWSTR lpApplicationName, LPWSTR lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes, LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles, DWORD dwCreationFlags, LPVOID lpEnvironment, LPCWSTR lpCurrentDirectory, LPSTARTUPINFOW lpStartupInfo, LPPROCESS_INFORMATION lpProcessInformation)
{
	...
	if (StringHlp::StrContainsW(lowStr, L"shutdown.exe", NULL)) {
		if (MessageBox(NULL, L"極域電子教室試圖關機或重新啟動，是否允許極域繼續操作？", L"JiYu Killer 防護警告", MB_ICONEXCLAMATION | MB_YESNO) == IDNO) canContinue = false;
	} 
...
INT_PTR CALLBACK JiYuWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	*/
	...
	else if (message == WM_DESTROY)
	{
		...
		case IDM_HELP_GB: {
			MessageBox(hWnd, L"此右鍵選單係 JiYuTrainer 為方便您控制廣播全螢幕與視窗模式而新增。\n若您需要開啟極域的原生右鍵選單，請在視窗中央按滑鼠右鍵。\n更多說明請參閱 JiYuTrainer 說明文件。", L"JiYuTrainer 提示", MB_ICONINFORMATION);
			return 0;
		}
		case IDM_HELP_GB2: {
		...
		case SC_CLOSE: {
			if (MessageBox(hWnd, L"您真的要關閉廣播視窗吗？", L"JiYuTrainer - 提示", MB_YESNO | MB_ICONEXCLAMATION) == IDYES)
			{
				CloseWindow(hWnd);
				DestroyWindow(hWnd);
			}
