...
NTSTATUS KxInitProtectProcess()
{
	if (!protectInited) {
		...
		opReg.ObjectType = PsProcessType;//處理程序
		opReg.Operations = OB_OPERATION_HANDLE_CREATE | OB_OPERATION_HANDLE_DUPLICATE;
		opReg.PreOperation = (POB_PRE_OPERATION_CALLBACK)&KxObPreCall; //註冊回呼函式指標
...
{
	if(obHandle) _ObUnRegisterCallbacks(obHandle); //取消註冊回呼
	if (protectInited) {
...
NTSTATUS NTAPI Hook_ZwTerminateProcess(__in_opt HANDLE ProcessHandle, __in NTSTATUS ExitStatus)
{
	...
	if (KxIsProcessProtect(uPID))//非目前處理程序 PID，允許用戶端自行結束
	{
