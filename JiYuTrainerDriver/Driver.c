...
#endif
	ldr->Flags |= 0x20;//繞過 MmVerifyCallbackFunction。
...
NTSTATUS IOControlDispatch(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	PIO_STACK_LOCATION  IoStackLocation = IoGetCurrentIrpStackLocation(Irp);  //IRP 堆疊  
	...
	IoCompleteRequest(Irp, IO_NO_INCREMENT);  //將 IRP 回傳給 IO 管理員
	return Status; //Ring3 DeviceIoControl() 回傳值
}
...
NTSTATUS ZeroKill(ULONG_PTR PID)   //X32  X64
{
	...
	if (NT_SUCCESS(ntStatus))
	{
		...
		KeStackAttachProcess(Eprocess, pKs);//附加至處理程序虛擬空間
...
NTSTATUS KxTerminateThreadApc(PETHREAD Thread)
{
	...
	if (Thread == PsGetCurrentThread()) {
		...
		if (PspExitThread == NULL)
			//使用 PspTerminateThreadByPointer 結束
